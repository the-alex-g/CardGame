	...
	for body in get_overlapping_bodies():
		if body is Target:
			if body.team_index != team_index and not body.is_dead:
				if (closest_target == null) or (global_position.distance_squared_to(body.global_position) < global_position.distance_squared_to(closest_target.global_position)):
					closest_target = body
...
func _manage_target_died_signals(new_target:Target)->void:
